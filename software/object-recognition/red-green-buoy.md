...
A minimum of three range-finding sensors, such as a [VEX Ultrasonic](../../vex-electronics/vex-sensors/ultrasonic.md) or Sharp IR rangefinder, must be used for proper implementation of the algorithm. Up to seven sensors can be used for the best possible quality, but it is also possible to do a decent job with only two sensors.
