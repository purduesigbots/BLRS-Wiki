...
A bang-bang control which only uses the current value, PID control schemes also may utilize information about the past values \(the _integral_\) and predicted future values \(the _derivative_\) to optimize control.
...
One might try to improve this scheme by adding additional [intermediate states](https://app.gitbook.com/@blrs/s/blrs-wiki/software/finite-state-machine/@merged) with the oven elements on at some partial amount, but this is difficult to maintain and adapt. A more intelligent scheme will use the other information that every analog sensor tells a smart controller:
...
For even better PID control, if some other information about the process is known that allows more accurate estimation of future values, that equation can be substituted for the deriative term. In the stove example mentioned above, one might install a second sensor to measure the room temperature and use Newton's Law of Cooling to estimate the rate at which heat would be lost to the environment based on the current temperature.
